# FACTS — verified API facts and open observations

**This is the project's standing technical reference, and it is the file other
documents cite.** It was §09 and §10 of `P0_RECORD.md`, which meant the ledger
every milestone writes into was buried inside a document named after the first
one. Lifting it out changes nothing in it: the text below is the original,
moved.

**§09 is verified.** Each entry was read from a primary source — a pinned header,
the Windows SDK, Microsoft's documentation — or observed on the rig, usually
after a guess proved wrong. Entries that turned out to be wrong are struck
through and corrected in place, never deleted; the reasoning that produced an
error is more useful than a document that has always been right.

**§10 is not verified.** It is things seen and not explained. Nothing in it
blocks anything. It exists so that a later fault is checked against it before
someone concludes they caused it.

Both stay live. A fact lands here regardless of which milestone produced it,
because splitting knowledge by milestone means a reader has to know which
document to search before they know what they are searching for.

Chronology lives in `RECORD.md`; the rig and the vendor stack live in
`../VENDOR_LOCK.md`.
...
### Additions from P4.0 → P4.2 — closed 2026-09-04/05

**The stream, the neural stage on it, and the payload — closed 2026-09-04/05 (P4.0, P4.1, P4.2)**

- **DEPTH IS NOT READ ON THIS PATH, AND THAT SETS THE PAYLOAD FOR THE WHOLE
  ARCHITECTURE.** Two evaluates on one captured frame, identical in every respect
  except whether a depth texture was bound: output **byte-identical over all
  3,686,400 pixels** at 2560×1440.
  **The depth was a front-to-back gradient, not a clear, and that is what makes
  it decisive.** A cleared depth carries no more information than no depth, so an
  identical result would have been ambiguous between *ignores depth* and *a flat
  depth means the same as none*. A sweeping plane removes that reading: a feature
  that sampled depth could not return the same bytes for both. So this is **not**
  "depth is optional" — it is not being sampled.
  **Consequence:** the per-frame payload is **colour only**. Combined with
  motion vectors never needing to cross — QuantMotion derives flow from colour
  *on GPU 1* at 0.13–0.17 ms, and the reference tool runs that substitution with
  zero failures over 3000 evaluates — **nothing but the colour buffer crosses the
  link**, and the ~2× payload every bandwidth note in this file was budgeting for
  does not exist. This holds at any link width.
  **SCOPE:** `preset=0` with the parameter set this add-on writes. The reference
  tool runs `guidanceMode=2` with `depthInterval=4`, which reads as it
  *supplying* depth on a cadence — so either its preset differs or guidance mode
  gates depth sampling. What is established is that **this configuration** does
  not read depth, not that DLSS-NR never does.
- **A single NGX feature handle serves a continuous stream.** 600 consecutive
  game frames, one handle, **600 evaluates, 0 failures**, at native resolution
  and native format. P3.2 had shown 16; this is the same claim at stream length
  and with live content.
- **`DLSSNR.Reset` becomes a first-frame-only key once frames stream.** Every
  probe before P4.1 set it on every evaluate, because each was an independent
  experiment and history between them would have contaminated the control. In a
  stream `dlssnr_prev_output` is temporal history and is meant to carry, so Reset
  is set on the first frame and cleared after. This is the first code in the
  project that lets NR accumulate across frames; a smeared or ghosted output is
  the symptom to look for, and setting it back to 1 is the first diagnostic.
- **DLSS-NR running on every frame costs the render GPU nothing measurable.**
  Producer pace with the neural stage attached sits in the same band as without
  it, across every properly-configured run. This is the claim the project exists
  to test, and it is observed end to end with nothing synthetic in the chain.
- **NGX WORKS ON A DISPLAY-ATTACHED ADAPTER.** The question deferred at the end
  of P2. With the bridge target reading `outputs=1`, `CreateFeature` succeeded
  (127 / 94 ms) and the stream ran 600 evaluates with 0 failures. Every prior
  result in this project was taken on a headless adapter.
- **Adapter selection survives a second display-attached GPU.** `game
  luid=0x000114B7`, target `0x00012827`, unchanged across every display
  configuration tried. Selection is swapchain-LUID exclusion and does not consult
  outputs, and the risk that a second display-attached GPU would move UE5's
  render adapter did not materialise.
- **`outputs=` follows the Windows main-display setting, not the cable.** Same
  cabling, two runs: main display 1 gave `outputs=1` on each adapter; main
  display 2 gave `outputs=2` on the game's adapter and `outputs=0` on the
  bridge's. Windows reassigns which adapter *owns* the outputs. **Never read
  `outputs=` as a statement about what is physically plugged in.**
- **A seal written into the payload's own buffer is what makes a stream
  checkable.** 64 bytes at the head of each ring slot, copied into the slot ahead
  of the pixels in the same command list, so submission order guarantees it lands
  first. Carries frame index, submit QPC, geometry and slot index. Identity,
  ordering, age and contract mismatches all fall out of comparing it against what
  was expected; none of them is reachable by a one-shot probe.
- **The producer-count fence generalises P2.0's ordering rule.** One shared fence
  whose value *is* the count of frames whose copies are known submitted,
  signalled on the game's queue **one event after** the copies were recorded. A
  signal in the same event sits ahead of our own copies and clears before the
  data exists.
- **A consumer that reads each frame once must not call a repeated index
  "reuse".** `P1_INSTRUMENT.md` §01 defines a reuse counter for a *sampling*
  consumer. In an event-driven consumer that reads each frame exactly once, an
  index equal to the previous one means the slot still held the old seal when the
  fence said the new one had landed — **a stale read, not expected behaviour**.
  The first build counted it as reuse and would have filed a real synchronisation
  fault under a healthy-looking number.
- **THE CONSUMER'S CADENCE IS THE BRIDGE SWAPCHAIN'S VSYNC, AND THAT IS A
  DEFECT.** Polling once per bridge present ties the checker to whichever display
  GPU 1 is attached to: 3.23× the producer's rate on a 210 Hz display, **1.01×**
  on a 60 Hz one. Nothing overran — each poll drains the whole backlog — but the
  margin is gone. Mitigated by raising ring depth 3 → 6 and warning by name when
  the ratio falls below 1.5. The fix is a consumer that blocks on the shared
  fence, which needs it off the bridge thread; the stream mutex is also taken by
  the event handler **on the game's render thread every frame**, so a consumer
  thread holding it across a GPU wait would stall the game. That is the one
  hazard in this add-on that can reach the application.
- **Fault injection selected from a file beside `dxgi.dll`, absent = clean.**
  Proven on the rig for `pitch`, `alias` and `magic`, each producing its own
  named diagnosis and nothing else. **An unknown fault name must be refused
  loudly**: accepting one that the build cannot inject produces a run with
  all-zero counters that the fault-injected warning then reports as a checker
  that failed to trip — a false accusation against working code.
- **A rejected seal must not be allowed to manufacture an ordering fault.** A
  `BAD MAGIC` at frame N leaves `last_seen` at N−1, so frame N+1 computes gap=2
  and reports a drop that did not happen. One injected corruption, two counters,
  and the producer blamed for a consumer-side fault. One gap check is suppressed
  after a rejection and the suppression is itself counted.


### Additions from P5 / P6 — closed 2026-09-05

### There is one DLSS-NR model in this snippet build

From `nvngx_dlssnr_310_8_0.log`, verbatim:

```
[CG2RNetworkManager::CreateNetwork:323] DLSSNR: 1 config(s) available:
[CG2RNetworkManager::CreateNetwork:327] DLSSNR:   [0] CC_Control_History_Blend_
    Quantize_With_Teacher_honest_tench_2026_07_04_22_30_weights (backbone: crazy-cuckoo)
[CG2RFindWeightByPreset:88]  DLSSNR: preset 0 is not available in this DLL build;
    falling back to shipping default preset 1
```

- It is a **preset**, not a model. `CG2RFindWeightByPreset` resolves a preset
  number to a weight set. "Model A/B/C" in RenoDX's panel is a label over this.
- **This DLL ships exactly one.** A model dropdown wired to it would resolve to
  the same weights whatever was picked — indistinguishable from a knob with a
  subtle effect.
- The check for later: that `N config(s) available:` line enumerates them. If it
  ever says more than 1, a preset knob becomes worth wiring.
- Weight heap is **140.9 MB** per network, and `Released network resources after
  FINAL feature release` implies it is refcounted and shared across features, not
  duplicated per handle. Four handles cost four sets of history buffers, one set
  of weights. Worth confirming with a VRAM read rather than assuming.
...
The code explains why: the snippet's own `Init_Ext` result is discarded, and
`CreateFeature` / `ReleaseFeature` / `EvaluateFeature` all resolve to the
**snippet**, not the core (`used:snippet` on the export line every launch). A
core `Init` failure never touches the path that does the work.

**Consequence for the instrument:** the P1.0c warning text says *"no NGX result
from this run may be recorded"*. That rule is over-broad — it was written for
runs where NGX did not work. It should be narrowed to void the result of the NGX
call that **failed or did not run**, not every NGX result in the launch.
Deliberately deferred to the shipping stage, along with checking whether
super-resolution DLAA survives the same error.
...
### §10 additions from P4.0 → P4.2

- **A DIFFERENCE BETWEEN RUNS THAT DIFFER IN MORE THAN ONE VARIABLE IS NOT A
  FINDING, AND THIS PROJECT SPENT LAUNCHES RELEARNING IT.** On 2026-09-05 a 29%
  frame-rate gap between two stream runs was attributed to DWM cross-adapter
  copies of the headless bridge window competing with the payload for the link —
  a mechanism §09 of this file genuinely documents. The runs differed in
  **resolution, bridge present rate and scene** as well as topology. Two clean
  runs at a properly-set 1920×1080 showed the opposite: **headless 47.7 fps
  against display-attached 44.8 fps.**
  Three things make this worth a §10 entry rather than a quiet correction.
  The rule that would have caught it was already written, in
  `P1_INSTRUMENT.md` §08, by the same author who then spent the launches — **a
  rule in a document does not fire on its own**, and the moment to apply it is
  when a number looks interesting, which is also the moment it feels least
  necessary. The mechanism being real and documented made the story *more*
  convincing, not less. And **the operator called it before the data did** —
  "personally I think we are chasing a ghost"; when the person running the rig
  says a measurement is not worth taking, that is evidence rather than an
  objection to answer.
- **The 2K 210 Hz panel reports 1920×1200 when asked for 1920×1080.** One run
  was taken in that stretched non-native mode before it was noticed, which is a
  second reason that run is not comparable to anything. Check the resolution the
  probe logs (`stream arm ... source=WxH`) against the resolution intended,
  rather than against what was typed into a settings dialog.
- **A second display-attached GPU breaks alt-tab between the game and the
  bridge window.** Observed while the game ran on the display-attached bridge
  card: neither window could be reached by alt-tab. Not diagnosed; recorded
  because the display path will have to deal with window management and this is
  the first sign of what it will run into.
- **NGX writes a second numbered log file when a second session is opened in
  the process.** `nvngx_01.log` appearing alongside `nvngx.log` is expected once
  the stream's neural stage opens its own session, not a fault signal. Do not
  read the file's existence as an error without reading its contents.
