        ...
        if (have_device)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // activation. The window is created without WS_VISIBLE
                    // because a window created visible takes foreground from
                    // the game, which drops it out of borderless-fullscreen.
                    //
                    // ShowWindow's return value is NOT success or failure: it
                    // is nonzero if the window was PREVIOUSLY VISIBLE and zero
                    // if it was previously hidden. This window is always
                    // previously hidden, so a correct call always returns
                    // FALSE with GetLastError() == 0. Checking it produced a
                    // spurious error line in every rig log. There is nothing
                    // to check here.
                    //
                    // SW_SHOWNOACTIVATE prevents activation, not Z-order: the
                    // window still enters at the top and covers the game
                    // without focusing it. Clicking the game restores the
                    // order. Cosmetic at P0; section 09 records why it is not
                    // cosmetic at M2.
